...
public class MapGenerator : MonoBehaviour
{
    ...
    public int mapChunkSize
    {
        get
        {
            ...
            {
                return MeshGenerator.supportedFlatshededChunkSizes[flatshadedChunkSizeIndex] - 1;
            }
            else
            {
    ...
    private void OnValidate()
    {
        if (terrainData != null)
        {
            terrainData.OnValuesUpadated -= OnValuesUpdated;
            terrainData.OnValuesUpadated += OnValuesUpdated;
        }
        if (noiseData != null)
        {
            noiseData.OnValuesUpadated -= OnValuesUpdated;
            noiseData.OnValuesUpadated += OnValuesUpdated;
        }
        if (textureData != null)
        {
            textureData.OnValuesUpadated -= OnTextureValuesUpdated;
            textureData.OnValuesUpadated += OnTextureValuesUpdated;
        }
    }
